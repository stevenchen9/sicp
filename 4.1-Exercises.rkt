...
;; (try try) the argument passed to (try) will be a procedure,
;; evaluating it will always return a value.

;; It is impossible to know if p is still running or running forever
;; without knowing intimately what p is and does, which would be
;; programmatically impossible. The best case would be to wait for a stack
;; overflow, then use that as a general "you used too many stack frames",
